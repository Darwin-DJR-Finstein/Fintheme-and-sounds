  ...
  class ThemeManager {
    ...
    async loadFromServer() {
      ...
      if (window.frappe && frappe.call) {
        try {
          const r = await frappe.call({ method: "finetheme_and_sound.api.get_active_theme" });
          result = r && r.message;
        } catch (_e) {
    ...
    async setActive(themeName, overrides = {}) {
      ...
      await frappe.call({
        method: "finetheme_and_sound.api.set_active_theme",
        args: {
    ...
    async resetToFrappeDefault() {
      if (window.frappe && frappe.call) {
        try {
          await frappe.call({ method: "finetheme_and_sound.api.clear_active_theme" });
        } catch (_e) {
    ...
    _watchFrappeToggle() {
      ...
      this._frappeObserver = new MutationObserver(() => {
        ...
        if (window.frappe && frappe.call) {
          frappe
            .call({ method: "finetheme_and_sound.api.clear_active_theme" })
            .catch(() => {});
        }
        this._notify();
      });
