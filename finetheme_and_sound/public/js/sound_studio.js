(function () {
  ...
  // and bundled with the app under finetheme_and_sound/public/sounds/, served by
  // Frappe at /assets/finetheme_and_sound/sounds/<event>-<n>.wav. Every event has
  ...
  const SND = (event, n) => `/assets/finetheme_and_sound/sounds/${event}-${n}.wav`;
  ...
  async function openSoundStudio() {
    ...
    const res = await frappe.call({ method: "finetheme_and_sound.api.get_user_sounds" });
    ...
    dialog.fields_dict.enabled.df.onchange = async () => {
      ...
      await frappe.call({
        method: "finetheme_and_sound.api.toggle_user_sounds",
        args: { enabled },
      });
    ...
    const injectResetAll = () => {
      ...
      $btn.on("click", async () => {
        ...
        if (!ok) return;
        await frappe.call({ method: "finetheme_and_sound.api.clear_all_user_sounds" });
        state.mapping = {};
  ...
  function bindRowEvents($root, dialog, state, rerender) {
    ...
    $root.on("click", "[data-action='frappe-default']", async function () {
      const key = $(this).closest(".sound-row").data("event");
      await frappe.call({
        method: "finetheme_and_sound.api.clear_user_sound",
        args: { event_key: key },
      });
    ...
    $root.on("click", ".preset-chip", async function () {
      ...
      try {
        await frappe.call({
          method: "finetheme_and_sound.api.set_user_sound",
